...
Key Constraint: Mobile-first, optimized for 3G/4G networks, utilizing serverless architecture. 2. Technology Stack
...
Icons: Lucide-Svelte 3. Directory Structure
code
Text
src/
├── lib/
...
├── drizzle.config.ts
└── svelte.config.js 4. Database Schema (Drizzle)
...
bookings: id (UUID), listing_id, renter_id, start_date, end_date, total_price, status. 5. Implementation Details (Vercel Specific)
...
Logic: Upload file -> Return https://storage.googleapis.com/BUCKET_NAME/tools/FILENAME. 6. Development Workflow for AI Agent
Initialize:
...
Use Tailwind grid for the main feed. 7. Migration Commands
...
npx drizzle-kit migrate 8. Specific Visuals
